...
def test_all_case(model, image_list, num_classes, patch_size=(96, 96, 96), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=0, nms=0):
    ...
    for image_path in loader:
        ...
        score_list.append(single_metric)
        
        if save_result:
            nib.save(nib.Nifti1Image(prediction.astype(np.float32), np.eye(4)), test_save_path +  "%02d_pred.nii.gz" % ith)
            nib.save(nib.Nifti1Image(score_map[0].astype(np.float32), np.eye(4)), test_save_path +  "%02d_logits.nii.gz" % ith)
...
def test_single_case(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    logit_map = np.zeros((num_classes,) + image.shape, dtype=np.float32)
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                y = y.cpu().data.numpy()
                y = y[0,1,:,:,:]
                y_logit = y1.cpu().numpy()[0]
                score_map[:, xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]] \
                  = score_map[:, xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]] + y
                logit_map[:, xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] += y_logit
                cnt[xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]] \
                  = cnt[xs:xs+patch_size[0], ys:ys+patch_size[1], zs:zs+patch_size[2]] + 1
    score_map = score_map/np.expand_dims(cnt,axis=0)
    logit_map = logit_map / np.expand_dims(cnt, axis=0)
    label_map = (score_map[0]>0.5).astype(np.int64)
    if add_pad:
        label_map = label_map[wl_pad:wl_pad+w,hl_pad:hl_pad+h,dl_pad:dl_pad+d]
        score_map = score_map[:,wl_pad:wl_pad+w,hl_pad:hl_pad+h,dl_pad:dl_pad+d]
        logit_map = logit_map[:,wl_pad:wl_pad+w,hl_pad:hl_pad+h,dl_pad:dl_pad+d]
    return label_map, logit_map
