...
parser.add_argument('--consistency_rampup', type=float, default=40.0, help='Rampup duration (epochs-like units)')

# BCP / CutMix-style masking
parser.add_argument('--u_weight', type=float, default=0.5, help='Weight of unlabeled pixels (not directly used here)')
parser.add_argument('--mask_ratio', type=float, default=2/3, help='Ratio of context mask')

# ------------------- NEW: principled improvements -------------------
...
parser.add_argument('--entropy_weighting', type=int, default=1, help='Use entropy-based soft weights (1 yes, 0 no)')
...
parser.add_argument('--lambda_dice_u', type=float, default=0.5, help='Weight for soft Dice consistency term on unlabeled')
...
def get_plab_conf_prob(logits):
    """
    logits: (B, C, D, H, W)
    returns:
      plab: (B, D, H, W) long
      conf: (B, D, H, W) float (max prob)
      probs: (B, C, D, H, W) float
    """
    probs = F.softmax(logits, dim=1)
    conf, plab = torch.max(probs, dim=1)
    return plab.long(), conf, probs


def voxelwise_kl_temp(student_logits, teacher_logits, T=2.0):
    """
    Temperature-scaled voxelwise distillation:
      KL( softmax(z_t/T) || softmax(z_s/T) ) * T^2
    returns: (B, D, H, W) KL map
    """
...
def entropy_weight(probs, gamma=2.0):
    """
    probs: (B, C, D, H, W)
    returns: (B, D, H, W) weight in [0,1], higher = more reliable
    w = (1 - H/ln(C))^gamma
    """
    eps = 1e-8
    C = probs.size(1)
    ent = -(probs * torch.log(probs.clamp_min(eps))).sum(dim=1)     # (B,D,H,W)
    ent_norm = ent / np.log(C)                                      # in [0,1]
...
def soft_dice_consistency(student_probs, teacher_probs, weight_map=None, eps=1e-6):
    """
    student_probs, teacher_probs: (B, C, D, H, W)
    weight_map: (B, D, H, W) or None
    returns scalar loss: 1 - mean_c Dice_c
    """
    ...
    if weight_map is not None:
        w = weight_map.view(B, 1, -1)  # broadcast across classes
        s = s * w
        t = t * w

    inter = (s * t).sum(dim=2)                       # (B,C)
    denom = s.sum(dim=2) + t.sum(dim=2)              # (B,C)
...
def rampup_factor(iter_num, rampup_iters):
    """
    Smooth ramp-up from 0 to 1 over rampup_iters, using sigmoid ramp.
    """
    if rampup_iters <= 0:
        return 1.0
    x = np.clip(iter_num / float(rampup_iters), 0.0, 1.0)
    # Reuse the classic sigmoid-like ramp used in SSL
    # convert x to "epoch-like" scale [0, rampup] for ramps.sigmoid_rampup
    return float(ramps.sigmoid_rampup(x * rampup_iters, rampup_iters))


def mix_consecutive_pairs(tensor, mask):
    """
    Pairwise mixing for consecutive pairs: (0,1), (2,3)...
    tensor: (B, ...)
    mask:   (B, 1, D, H, W) or (B, D, H, W) broadcastable to tensor
    """
    B = tensor.size(0)
    ...
    # Expand mask to tensor shape for broadcasting
    if mask.dim() == tensor.dim() - 1:
        # mask (B,D,H,W) -> add channel dim if tensor has it (e.g., logits are (B,C,D,H,W))
        m = mask
...
def pre_train(args, snapshot_path):
    ...
    for _ in iterator:
        for _, batch in enumerate(trainloader):
            ...
            iter_num += 1
            logging.info(f'pre iter {iter_num} | loss={loss.item():.4f} | dice={loss_dice.item():.4f} | ce={loss_ce.item():.4f}')
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    for _ in iterator:
        for _, batch in enumerate(trainloader):
            ...
            # ------------------- unlabeled (teacher=encoder clean, student=decoder perturbed) -------------------
            with torch.no_grad():
                t_logits, _ = encoder_model(unimg)
                t_plab, t_conf, t_probs = get_plab_conf_prob(t_logits)
                conf_mask = (t_conf > args.pl_conf_thres).float()

                if args.entropy_weighting == 1:
                    w_ent = entropy_weight(t_probs, gamma=args.entropy_gamma)  # (B,D,H,W)
                    conf_mask = conf_mask * w_ent

            s_logits_dec, feat_dec_drop = decoder_model(unimg, feature_perturb='dropout', p=0.6)

            # KL distillation map (temperature-scaled)
            kl_map = voxelwise_kl_temp(s_logits_dec, t_logits, T=args.temp)  # (B,D,H,W)
            u_kl = (kl_map * conf_mask).sum() / (conf_mask.sum() + 1e-6)

            # Soft Dice consistency between student probs and teacher probs (also weighted)
            s_probs = F.softmax(s_logits_dec, dim=1)
            t_probs_T1 = F.softmax(t_logits, dim=1)  # teacher probs at T=1 for Dice consistency
            u_dice = soft_dice_consistency(s_probs, t_probs_T1, weight_map=conf_mask)

            # ------------------- feature consistency for encoder branch (keep your idea) -------------------
            with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                inf_plab, inf_conf, inf_probs = get_plab_conf_prob(inf_logits)

                # Context mask (pairwise region mixing)
                img_mask, loss_mask = context_mask(unimg, args.mask_ratio)

                # agreement + confidence gating (stable)
                enc_logits_ulb, _ = encoder_model(unimg)
                dec_logits_ulb, _ = decoder_model(unimg)
                enc_plab_ulb, _, _ = get_plab_conf_prob(enc_logits_ulb)
                dec_plab_ulb, _, _ = get_plab_conf_prob(dec_logits_ulb)
            ...
            # ------------------- pairwise mixing (inputs + pseudo labels) -------------------
            ...
            # mix reliability masks in the same way as your original logic
            ...
            # CPS loss on mixed image (hard targets are OK here; reliability is soft)
