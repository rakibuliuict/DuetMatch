...
def add_input_noise(x, noise_std=0.1):
    noise = torch.randn_like(x) * noise_std
    return x + noise

def add_feature_gaussian_noise(features, noise_std=0.1):
    noise = torch.randn_like(features) * noise_std
    return features + noise

def feature_pertubation(features, method='gaussian', p=0.5):
    perturbed_features = None
    if method=='gaussian':
        perturbed_features = add_feature_gaussian_noise(features)
    elif method=='dropout':
        dropout_layer = nn.Dropout3d(p=p)
        perturbed_features = dropout_layer(features)
        
    return perturbed_features

def input_pertubation(inputs, method='gaussian', p=0.5):
    perturbed_inputs = None
    if method=='gaussian':
        perturbed_inputs = add_feature_gaussian_noise(inputs)
    elif method=='dropout':
        dropout_layer = nn.Dropout3d(p=p)
        perturbed_inputs = dropout_layer(inputs)
...
class VNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        ...
        dim_in = 16
        feat_dim = 32
        self.pool = nn.AvgPool3d(3, stride=2)
    ...
    def forward(self, input, input_perturb=None, feature_perturb=None, p=0.5):
        
        if input_perturb != None:
            input = input_pertubation(input, input_perturb, p=p)
        
        features = self.encoder(input)
        
        if feature_perturb != None:
            features[4] = feature_pertubation(features[4], feature_perturb, p=p)
